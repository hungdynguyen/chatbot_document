...
class DocumentParser:
    ...
    def parse_excel(self, file_path: Path) -> List[Document]:
        """
        [DEMO MODE] Parses each table in each sheet into a single Markdown Document.
        """
        documents = []
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                # Chuyển đổi toàn bộ DataFrame (bảng) thành một chuỗi Markdown
                markdown_text = df.to_markdown(index=False)
                
                page_content = f"Dữ liệu từ file '{file_path.name}', sheet '{sheet_name}':\n\n{markdown_text}"
                
                metadata = {
                    "source": str(file_path),
                    "file_type": "excel",
                    "sheet_name": sheet_name,
                    "content_type": "full_table_markdown"
                }
                documents.append(Document(page_content=page_content, metadata=metadata))
                
        except Exception as e:
            print(f"Error parsing Excel for demo {file_path}: {e}")
    ...
    def parse_word(self, file_path: Path) -> List[Document]:
        """
        [DEMO MODE] Parses each table in the document into a single Markdown Document.
        """
        documents = []
        try:
            ...
            for table_idx, table in enumerate(doc.tables):
                ...
                for row in table.rows:
                    if not any(cell.text.strip() for cell in row.cells): continue
                    table_data.append([cell.text.strip() for cell in row.cells])
                
                if not table_data: continue
                
                # Chuyển đổi dữ liệu bảng thành chuỗi Markdown
                ...
                page_content = f"Bảng {table_idx} từ file '{file_path.name}':\n\n{markdown_table}"
                metadata = {
                    "source": str(file_path),
                    "file_type": "word",
                    "content_type": "full_table_markdown",
            ...
            print(f"Error parsing Word for demo {file_path}: {e}")
